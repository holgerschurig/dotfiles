...
--- Tagbox widget module for awful
-- awful.widget.tagbox
local tagbox = { mt = {} }
tagbox.filter = {}

function tagbox.tagbox_label(t, args)
    if not args then args = {} end
    local theme = beautiful.get()
    local fg_focus = args.fg_focus or theme.tagbox_fg_focus or theme.fg_focus
    local bg_focus = args.bg_focus or theme.tagbox_bg_focus or theme.bg_focus
    local fg_urgent = args.fg_urgent or theme.tagbox_fg_urgent or theme.fg_urgent
    local bg_urgent = args.bg_urgent or theme.tagbox_bg_urgent or theme.bg_urgent
    local bg_occupied = args.bg_occupied or theme.tagbox_bg_occupied
    local fg_occupied = args.fg_occupied or theme.tagbox_fg_occupied
    local bg_empty = args.bg_empty or theme.tagbox_bg_empty
    local fg_empty = args.fg_empty or theme.tagbox_fg_empty
    local tagbox_squares_sel = args.squares_sel or theme.tagbox_squares_sel
    local tagbox_squares_unsel = args.squares_unsel or theme.tagbox_squares_unsel
    local tagbox_squares_sel_empty = args.squares_sel_empty or theme.tagbox_squares_sel_empty
    local tagbox_squares_unsel_empty = args.squares_unsel_empty or theme.tagbox_squares_unsel_empty
    local tagbox_squares_resize = theme.tagbox_squares_resize or args.squares_resize or "true"
    local tagbox_disable_icon = args.tagbox_disable_icon or theme.tagbox_disable_icon or false
    local font = args.font or theme.tagbox_font or theme.font or ""
    ...
    if t:clients() == 0 and t.selected and tagbox_squares_sel_empty then
        bg_image = tagbox_squares_sel_empty
        bg_resize = tagbox_squares_resize == "true"
    elseif not is_selected then
        local cls = t:clients()
        if #cls > 0 then
            if tagbox_squares_unsel then
                bg_image = tagbox_squares_unsel
                bg_resize = tagbox_squares_resize == "true"
            end
            if bg_occupied then bg_color = bg_occupied end
            if fg_occupied then fg_color = fg_occupied end
        else
            if tagbox_squares_unsel_empty then
                bg_image = tagbox_squares_unsel_empty
                bg_resize = tagbox_squares_resize == "true"
            end
    ...
    text = text .. "</span>"
    if not tagbox_disable_icon then
    ...
    return text, bg_color, bg_image, not tagbox_disable_icon and icon or nil
end

local function tagbox_update(s, w, buttons, filter, data, style, update_function)
    local tags = {}
    ...
    local function label(c) return tagbox.tagbox_label(c, style) end
...
function tagbox.gettag(widget)
    return common.tagwidgets[widget]
end

--- Create a new tagbox widget. The last two arguments (update_function
-- and base_widget) serve to customize the layout of the tagbox (eg. to
...
-- @param screen The screen to draw tagbox for.
...
-- font The font.
function tagbox.new(screen, filter, buttons, style, update_function, base_widget)
    ...
    local u = function (s)
        if s == screen then
            tagbox_update(s, w, buttons, filter, data, style, uf)
        end
    end
...
function tagbox.filter.noempty(t, args)
...
-- @param args unused list of extra arguments.
-- @return true
function tagbox.filter.all(t, args)
    return true
end

function tagbox.mt:__call(...)
    return tagbox.new(...)
end

return setmetatable(tagbox, tagbox.mt)
